...
# The record types we care about.
_INTERESTED_RECORDS = {
...
def _get_ops_by_type(commit: models.ComAtprotoSyncSubscribeRepos.Commit) -> defaultdict:
  """
  Extracts operations from the commit and groups them by record type.
  """
...
async def run(name: str, operations_callback, app: FastAPI, stream_stop_event: asyncio.Event) -> None:
  """
  Continuously runs the subscription client. On reaching commit.seq % 1000 == 0,
  it updates state and stops the feed.
  """
...
async def _run(name: str, operations_callback, app: FastAPI, stream_stop_event: asyncio.Event) -> None:
  """
  Runs a single instance of the subscription client.
  Retrieves (or creates) the subscription state from PostgreSQL and starts the client.
  Stops the client automatically once commit.seq % 1000 == 0.
  """
  # Retrieve the stored subscription state.
  ...
  # If no state exists, create an initial record.
  if not state:
    async with app.state.pool.acquire() as conn:
    ...
    async def on_message_handler(message: firehose_models.MessageFrame) -> None:
      ...
      # Every 1000 events, update the stored cursor and stop the feed.
      if commit.seq % 1000 == 0:
        ...
        # Stop the feed after processing this batch.
        client.stop()
      ...
      # Process operations with the provided callback.
