...
async def operations_callback(ops: defaultdict, app: FastAPI) -> None:
  ...
  """
  def _post_is_archive(record: "models.AppBskyFeedPost.Record", threshold: int = 1) -> bool:
  ...
  def _filter_for_new_videos(record: "models.AppBskyFeedPost.Record") -> bool:
    if config.IGNORE_ARCHIVED_POSTS and _post_is_archive(record):
      return True
  ...
  def _pack_post(created_post: defaultdict, record: "models.AppBskyFeedPost.Record") -> PostModel:
    post = PostModel()
  ...
  for created_post in ops[models.ids.AppBskyFeedPost]['created']:
    ...
    if _filter_for_new_videos(record):
      continue
    
    # If feet then keep
    if "feet" in record.text.lower():
      posts_to_create.append(_pack_post(created_post, record))
